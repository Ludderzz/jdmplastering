...
const WhatsAppWidget = () => {
  ...
  const phoneNumber = "447896560566"; // Replace with your actual WhatsApp number (include country code)

  const handleSendMessage = (e) => {
    e.preventDefault();
    // Encode the message for a URL
    const encodedMsg = encodeURIComponent(message || "Hi JDM Plastering, I'd like to get a quote.");
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[9999] flex flex-col items-end font-sans">
      ...
      {/* --- Chat Bubble / Window --- */}
      {isOpen && (
        <div className="mb-4 w-[320px] sm:w-[380px] bg-white rounded-3xl shadow-2xl border border-slate-100 overflow-hidden animate-in slide-in-from-bottom-10 duration-300">
          {/* Header */}
          <div className="bg-slate-900 p-6 text-white">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h4 className="font-bold text-lg leading-none">JDM Plastering</h4>
                <p className="text-xs text-slate-400 mt-1">Typically responds in 1 hour</p>
              </div>
              <button 
          ...
          <div className="p-6 bg-slate-50">
            <div className="bg-white p-4 rounded-2xl rounded-tl-none shadow-sm border border-slate-100 mb-4">
              <p className="text-sm text-slate-700 leading-relaxed">
                Hi there! 👋 <br />
                Looking for a quote or have a question about your project? Message us below.
              </p>
            </div>
            ...
            <form onSubmit={handleSendMessage} className="space-y-3">
              <textarea
                ...
                className="w-full p-4 rounded-xl border border-slate-200 focus:ring-2 focus:ring-amber-500 focus:border-transparent outline-none text-sm resize-none h-24"
                value={message}
                onChange={(e) => setMessage(e.target.value)}
              />
              <button 
                type="submit"
                className="w-full bg-[#25D366] hover:bg-[#128C7E] text-white font-black py-4 rounded-xl transition-all flex items-center justify-center gap-2 uppercase text-xs tracking-widest shadow-lg shadow-green-200"
              >
      ...
      >
        {!isOpen && (
          <span className="hidden sm:block pl-2 font-black text-slate-900 uppercase text-xs tracking-widest">
            Chat with us
          </span>
        )}
